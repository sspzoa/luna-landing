...
import { useProjects } from '@/shared/hooks/useApi';
...
export default function Projects() {
  ...
  return (
    <div className="flex flex-col justify-center items-center w-full">
      <Intro projects={projects} isLoading={isLoading} />
...
const Intro: React.FC<IntroProps> = ({ projects, isLoading }) => {
...
const ProjectsList: React.FC<ProjectsProps> = ({ projects, isLoading }) => {
  const years = useMemo(() => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 max-w-[1200px] w-full gap-x-8 gap-y-7">
          {[1, 2, 3, 4].map((i) => (
            <div
              ...
              <Skeleton className="w-[140px] h-[140px] flex-shrink-0" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 max-w-[1200px] w-full gap-x-8 gap-y-7">
        ...
        {filteredProjects.length === 0 && !isLoading && (
          <div className="col-span-full flex justify-center items-center p-10">
            <p className="text-18 font-medium opacity-70">{selectedYear}년에는 프로젝트가 없습니다.</p>
          </div>
        )}
      </div>
    </div>
