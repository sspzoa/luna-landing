...
import { useQnA } from '@/shared/hooks/useApi';
import type { QnA } from '@/shared/types';
import type React from 'react';
import { useState } from 'react';

export default function Qna() {
  const { data: qna = [], isLoading } = useQnA();

  return (
    <div className="flex flex-col justify-center items-center w-full">
      <QnaSection qna={qna} isLoading={isLoading} />
    </div>
  );
}
...
const QnaSection: React.FC<QnaProps> = ({ qna, isLoading }) => {
      ...
      <div className="relative flex flex-col gap-30 justify-center items-center w-full pt-48 pb-30 px-9">
        <div className="relative flex flex-col md:flex-row justify-between items-center w-full max-w-[1200px] min-h-[315px] gap-9">
          <div className="flex flex-col gap-4 shrink-0 self-start md:self-center z-10">
            ...
            <div className="flex flex-col gap-2 max-w-[435px]">
              <SkeletonText className="w-full" />
              <SkeletonText className="w-4/5" />
              <SkeletonText className="w-3/4" />
            </div>
          </div>
          <Skeleton className="self-end md:self-center w-[560px] h-[315px] rounded-3xl" />
        </div>
        <div className="w-full max-w-[1200px] flex flex-col gap-7">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="border-2 border-luna-dark-10 rounded-3xl overflow-hidden">
              <div className="flex flex-row items-center px-9 py-7 gap-4">
