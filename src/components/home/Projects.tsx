...
export default function Projects({ information, projects }: ProjectsProps) {
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center gap-12 py-20 sm:gap-20 sm:py-50">
      <div className="w-full max-w-[1272px] items-start justify-start px-5 sm:px-9">
      ...
      <div className="flex w-full flex-col items-center justify-center gap-3 overflow-hidden">
        {projectChunks.map((chunk, rowIndex) => (
          <div
            key={`row-${rowIndex}`}
            className="flex min-w-full flex-row items-center justify-center gap-3 sm:gap-4"
            style={{
            ...
            {chunk.map((project, index) => (
              <div
                key={project.id}
                className={`flex items-center justify-center rounded-full px-6 py-3 sm:px-15 sm:py-4 ${colorClasses[index % colorClasses.length]}`}>
                <p className="max-w-[160px] truncate text-16 font-semibold text-luna-white sm:max-w-[200px]">
      ...
      <div className="flex w-full max-w-[1272px] flex-col items-end justify-end gap-4 px-5 sm:px-9">
        <p className="text-right text-32 font-medium">
          <strong className="text-luna-purple">{information.projects}개의 프로젝트</strong>를 진행하며
          <br />
