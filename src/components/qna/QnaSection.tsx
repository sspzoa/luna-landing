  ...
  return (
    <div className="relative flex w-full flex-col items-center justify-center gap-30 px-9 pt-48 pb-30">
      <Container className="relative flex min-h-[315px] flex-col items-center justify-between gap-9 md:flex-row">
        <FadeIn className="z-10 flex shrink-0 flex-col gap-4 self-start md:self-center">
        ...
        <FadeIn delayMs={100} className="self-end md:absolute md:right-0 md:self-center">
          <iframe
            className="rounded-3xl border-2 border-luna-dark-10"
            width="560"
            height="315"
      ...
      <Container className="flex flex-col gap-7">
        {sortedQna.map((item, index) => {
          ...
          return (
            <FadeIn key={item.id} delayMs={Math.min(index * 40, 200)}>
              <div
                className={cn(
